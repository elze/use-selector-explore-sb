  ...
  switch (action.type) {
    case 'incrementCount':
      ...
      if (action.name === 'apple') {
        ...
        console.log(
          `reducer, case toggleButton: action.name === ${
            action.name
          } state = ${JSON.stringify(state)} newState = ${JSON.stringify(
            newState
          )} shallowEqual(state.apple, newState.apple) = ${shallowEqual(
            state.apple,
            newState.apple
          )} shallowEqual(state.mushroom, newState.mushroom) = ${shallowEqual(
            state.mushroom,
            newState.mushroom
          )}`
        );
